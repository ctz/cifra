 ...
 * `nonce` and `persn` (personalisation string).
 */
 ...
 * this function returns non-zero, the next `_gen` or `_gen_additional`
 * call will call abort.
 */
extern uint32_t cf_hash_drbg_sha256_needs_reseed(const cf_hash_drbg_sha256 *ctx);

/* .. c:function:: $DECL
 * Reseeds the generator with the given `entropy` and `addnl` (additional
 * data).
 */
