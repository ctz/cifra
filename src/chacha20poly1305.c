                   ...
                   uint8_t tag[static 16])
{
  ...
  cf_chacha20_init(&chacha, key, 32, nonce);
   ...
   * AAD || len_64(AAD) || cipher || len_64(cipher) */
  uint8_t lenbuf[8];
  cf_poly1305_update(&poly, header, nheader);
  write64_le(nheader, lenbuf);
  cf_poly1305_update(&poly, lenbuf, sizeof lenbuf);
